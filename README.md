...
Платформа автоматизирует рутинные обязанности системного аналитика и архитектора решений,
покрывая полный цикл управления требованиями — от приёмки «сырого» ТЗ до бизнес-кейса
и проектной документации для передачи в разработку.

| # | Задача | Что автоматизируется |
|---|--------|----------------------|
| 1 | **Быстрая и честная рецензия ТЗ** | AI-анализ полноты, корректности и непротиворечивости требований; риски, уточняющие вопросы к постановщику, критерии приёмки; честные метки `needs_review`/`confidence` вместо «всегда корректного» ответа |
| 2 | **Пакетная проверка требований** | До 50 ТЗ за один запуск со сводным отчётом и экспортом CSV |
| 3 | **Формализация требований** | Генерация URS/SRS по ГОСТ 34.602-2020 / ISO/IEC/IEEE 29148 / IEEE 830 |
| 4 | **Проектирование архитектуры** | Рекомендации по архитектуре, ADR, OpenAPI 3.1 и 8 типов диаграмм (C4/UML/ERD) с версионированием; рендер локально через Kroki |
| 5 | **Управление знаниями команды** | База знаний с RAG: загрузка документов, автоиндексация, ответы с цитатами-источниками; «данных недостаточно» вместо галлюцинаций |
| 6 | **Накопление опыта проектов** | Каталог типовых рисков (вероятность/влияние/митигации) и уроки (Lessons Learned), переиспользуемые в следующих проектах |
| 7 | **Экономическая оценка до разработки** | AI-декомпозиция требований на задачи, оценка трудозатрат по ролям, CAPEX/OPEX/ROI/payback, план/факт, экспорт бизнес-кейса в DOCX/PDF |
| 8 | **Прозрачность и аудит** | Каждая AI-операция пишется в `audit_runs` с причиной ручной проверки и фактической стоимостью LLM-вызовов |
| 9 | **Безопасность и автономность** | JWT + RBAC (admin/analyst/architect); режим `ENFORCE_LOCAL_ONLY` — полная работа offline на локальном Ollama |

## Ключевые бизнес-функции и процессы

| Бизнес-процесс | Кто использует | Как работает в системе | Результат |
|----------------|----------------|------------------------|-----------|
| **Рецензирование ТЗ** | Аналитик | Загрузка ТЗ → AI-проверка полноты/противоречий → риски, вопросы заказчику, критерии приёмки, метки `needs_review`/`confidence` → ручная проверка при необходимости | Заключение + экспорт DOCX/JSON/CSV |
| **Пакетный аудит требований** | Аналитик | До 50 ТЗ одним запуском → общий прогон → сводный отчёт и CSV | Отчёт по партии документов |
| **Генерация URS/SRS** | Аналитик | Формализация ТЗ по выбранному стандарту (ГОСТ/ISO/IEEE) → готовый документ + автоиндексация в базу знаний | Документы URS/SRS |
| **Архитектура и ADR** | Архитектор | Рекомендация архитектуры → ADR → OpenAPI → диаграммы C4/UML/ERD с версиями и rollback | Комплект проектной документации |
| **База знаний команды** | Вся команда | Загрузка статей → автоиндексация (keyword + FAISS) → вопросы с цитатами-источниками | Проверяемые ответы |
| **Риски и уроки** | Аналитик / команда | Автоизвлечение рисков из рецензий в каталог → ручное дополнение → export CSV | Каталог рисков и Lessons Learned |
| **Экономика проекта** | Аналитик / архитектор | Build-проект из ТЗ → декомпозиция задач → оценка часов → CAPEX/OPEX/ROI/payback → план/факт | Бизнес-кейс + экспорт DOCX/PDF |
| **Контроль LLM-бюджета** | Все роли | Прозрачная тарификация токенов по провайдерам; локальные модели (Ollama) — $0 | Фактические расходы в Dashboard |
| **Администрирование демо-среды** | Администратор | Управление пользователями; демо-данные одной кнопкой (`POST /seed/examples`) | Готовая витрина продукта |
...
cd backend
python -m venv .venv && .venv\Scripts\activate     # Windows (.venv/bin/activate — Linux/macOS)
pip install -r requirements.txt
cp ../.env.example .env                            # укажите LLM_PROVIDER (+ ключ) и APP_SECRET_KEY
...
> **Тестовые учётные записи:** `admin/admin123`, `analyst/analyst123`, `architect/architect123`.
> Демо-данные для всех разделов загружаются кнопкой **📥 Загрузить примеры для всех процессов**
> на странице ⚙️ Настройки (admin) или запросом `POST /seed/examples`.
...
FastAPI + 15 роутеров, 25 моделей, 21 сервис, **160 pytest-тестов**, React 18 + Vite, модуль
экономики, полный E2E-прогон на локальном LLM.

### Ближайшие шаги

- **v1.1** — webhook-уведомления при `needs_review`; расширение экспорта (шаблоны DOCX по ГОСТ)
- **v1.2** — shadcn/ui и TanStack Query во фронтенде; E2E-тесты фронтенда (Playwright)
- **v1.3** — интеграция фактических расходов с тайм-трекерами (Toggl/Harvest) для автосбора план/факта
- **v1.4** — импорт ТЗ из Jira/Confluence; live-рендер Mermaid-диаграмм в UI

### Стратегические направления

| Направление | Что это даёт бизнесу |
|-------------|----------------------|
| **AI-агент полного цикла** | Автономный агент «рецензия → доработка ТЗ → повторная рецензия» до прохождения критериев приёмки без участия человека |
| **Fine-tuned модель на корпоративных ТЗ** | Специализированный LLM с точностью выше универсальных на доменных ТЗ (v2.0) |
| **Портфельный Dashboard ROI** | Сводный ROI по всем build-проектам компании, приоритизация инвестиций (v2.0) |
| **Мультитенантность / SaaS** | Отдельные пространства команд и проектов, SSO/LDAP, изоляция данных |
| **MCP-интерфейс** | Подключение платформы как MCP-сервера к внешним AI-ассистентам и IDE |
| **Облачный деплой** | GitHub Actions → Azure/AWS: управляемый PostgreSQL, CI/CD, мониторинг |
| **Расширение стандартов** | ГОСТ 19 (ЕСПД), IDEF0/EPC-диаграммы, шаблоны отчётности для presale |
