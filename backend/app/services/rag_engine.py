...
Если ответ опирается на контекст — в sources должна быть хотя бы одна ДОСЛОВНАЯ цитата из
КОНТЕКСТА (копируй фрагмент как есть, не пересказывай и не сочиняй). Пустой sources допустим
только вместе с needs_review=true.
